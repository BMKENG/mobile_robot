# FlexBE States and Behaviors for mobile_robot
...
This raw repository has several folders and files with the generic name `mobile_robot`.
...
and change the relevant `mobile_robot` text to `my_new_project_name` as needed.
...
## Example States in `mobile_robot_flexbe_states`
...
## Example Behaviors in `mobile_robot_flexbe_behaviors`
...
[`copy_behavior`](mobile_robot_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run mobile_robot_flexbe_behavior copy_behavior` to see the usage guide. 
