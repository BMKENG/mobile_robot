...
class ROSNode(QThread):
    ...
    def run(self):
        self.node = rclpy.create_node('mobile_robot_gui_node')
        
        self.sub_cmd_vel = self.node.create_subscription(Twist, 'set_vel', self.cmd_vel_callback, 10)
        self.set_vel = None

        self.sub_pose = self.node.create_subscription(PoseWithCovarianceStamped, 'amcl_pose', self.pose_callback, 10)
        self.current_pose = None

        # 진행률 표시
        self.sub_remaining_waypoint = self.node.create_subscription(NaviProgress, 'remaining_waypoint', self.remaining_waypoint_callback, 10)
